...
# %% auto #0
__all__ = ['app', 'rt', 'sysp', 'models', 'model', 'h', 'js_ui', 'js_submit', 'MsgType', 'bubble', 'mk_prompt', 'setmodel',
           'dropdown', 'ask_llm', 'llm_rsp', 'send', 'chatpg']
...
def setmodel(name:str):
    global model
    model = name
    return Summary(model, cls='btn m-1', id="selmodel")

# %% ../nbs/chat.ipynb #a8097546
def dropdown():
    return Details(cls='dropdown dropdown-top')(
        Summary('Model', cls='btn m-1', id="selmodel"),
...
def ask_llm(msg:str):
    global h
    global model
    r = Chat(model=model, sp=sysp)(mk_prompt(msg, h))
